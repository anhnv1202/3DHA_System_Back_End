...
export class ChapterService {
  ...
  async updateQuizz(user: User, id: string, data: UpdateQuizzInChapterDTO): Promise<Chapter | null> {
    ...
    const isQuizzExist = currentChapter.questions.includes(quizz);
    if ((option === Option.ADD && isQuizzExist) || (option === Option.REMOVE && !isQuizzExist)) {
      throw new BadRequestException(isQuizzExist ? 'quizz-existed' : 'quizz-not-existed');
    }
    const updateOperation = option === Option.ADD ? { $push: { quizzs: quizz } } : { $pull: { quizzs: quizz } };
  ...
  //       throw new BadRequestException('permission-denied');
  //     }
  // const isLessonExist = currentChapter.lessons.includes(lesson);
  // if ((option === Option.ADD && isLessonExist) || (option === Option.REMOVE && !isLessonExist)) {
  //   throw new BadRequestException(isLessonExist ? 'lesson-existed' : 'lesson-not-existed');
  // }
  // const updateOperation = option === Option.ADD? { $push: { lessons: lesson } } : { $pull: { lessons: lesson } };
  // return await this.chapterRepository.update(id, updateOperation);
  // }
